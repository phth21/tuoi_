...
def ask_gemini(force=False):
    ...
    # Check ngập úng trước (ưu tiên cao nhất)
    if state['soil'] >= FLOOD_LEVEL:
        control_pump(False, "Safety Check - Ngập úng")
        return
    if not model: 
    ...
    Khẩn cấp (<{CRITICAL_LEVEL}%): {"Có" if is_emergency else "Không"}.
    ...
    - "timing": Mô tả ngắn gọn bao giờ tưới và độ ẩm dự đoán.
    - "reason": Lý do ngắn gọn.
    - Nếu đất < {CRITICAL_LEVEL}% → Bắt buộc decision: "ON".
    """
    try:
        ...
        if match:
            ...
            state['ai_timing'] = data.get('timing', 'Đang phân tích...')
            state['ai_target'] = data.get('target', 'Chưa xác định')
            state['ai_reason'] = data.get('reason', 'Đang xử lý...')
            state['last_ai_call'] = now 
            
            log_event("AI_DECISION", f"AI: {dec} | Target: {state['ai_target']} | {state['ai_reason']}")
            control_pump(dec == 'ON', "AI Logic")
            print(f"✅ AI Done: {dec} | {state['ai_reason']}")
        else:
            state['ai_timing'] = "Lỗi định dạng"
            state['ai_target'] = "?"
            state['ai_reason'] = "AI không trả JSON"
    except Exception as e:
        print(f"❌ AI Error: {e}")
        state['ai_reason'] = "Lỗi kết nối AI"
...
def control_pump(on, source="System"):
    ...
    if not on and "NGẬP ÚNG" in state['warning']:
        state['warning'] = ""
    broadcast()
        ...
        if msg.topic == PREFIX + "esp/data" and "H:" in payload:
            try:
                ...
                if state['soil'] >= FLOOD_LEVEL and state['pump']:
                    control_pump(False, "Safety - Ngập úng")
                
                # Tự động tắt khi đạt target (AUTO mode)
                elif state['mode'] == 'AUTO' and state['pump']:
                    target_str = state['ai_target']
                    nums = re.findall(r'\d+', target_str)
                    if nums:
                        target_val = int(nums[0])
                        if state['soil'] >= target_val + 2:
            ...
            if evt == 'select_region':
                ...
                print(f"🌍 GPS: {state['lat']}, {state['lon']}")
                threading.Thread(target=update_weather, daemon=True).start()
                
            elif evt == 'user_control' and state['mode'] == 'MANUAL':
                control_pump(bool(data['pump']), "Người dùng")
